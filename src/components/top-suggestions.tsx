...
const ScoreCircle: React.FC<{ score: number }> = ({ score }) => {
    ...
    return (
      <div className="relative h-10 w-10 flex-shrink-0">
        ...
        <span className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-bold text-sm ${colorClass}`}>
          {score}
        </span>
      </div>
    );
    ...
    return (
        <Card className={`shadow-lg ${cardClasses} flex flex-col`}>
            <CardHeader className="flex-row items-center gap-2 space-y-0 p-4 pb-2">
                ...
                <CardTitle className="text-sm font-bold text-foreground/80">{titleText}</CardTitle>
            </CardHeader>
            <CardContent className="p-4 pt-2 space-y-2 flex-1 flex flex-col justify-between">
